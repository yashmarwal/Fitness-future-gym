...
// text, and a QR code with the same hard-shadow accent-offset treatment.
// Previously this page was a much plainer version with no QR code at all.
export default async function MembershipCardPage() {
  ...
  return (
    <div className="px-gutter-mobile lg:px-gutter-desktop py-8 max-w-md mx-auto">
      ...
      <div className="bg-surface-container-low border border-surface-variant/40 shadow-hard-lg relative overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="relative bg-surface-container grid grid-cols-2 divide-x divide-surface-variant/40 border-l-4 border-primary-container">
          ...
          {qrDataUrl && (
            <div className="flex justify-center py-2">
              {/* Hard, offset accent rectangle behind the white QR panel —
                  the same brutalist shadow-hard language used across the
                  site's cards, in the accent color so it actually reads
                  against this card's dark background. */}
              <div className="relative">
                <div className="absolute top-1.5 left-1.5 w-full h-full bg-primary-container" />
                {/* eslint-disable-next-line @next/next/no-img-element -- data: URL, next/image can't optimize it */}
                <img src={qrDataUrl} alt="Membership QR code" className="relative w-36 h-36 bg-white p-2" />
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-4 border-t border-surface-variant/40">
            ...
            <span
              className={`font-label text-[10px] uppercase tracking-wide px-2 py-1 ${
