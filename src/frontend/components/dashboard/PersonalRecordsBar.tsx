...
// rounded icon circle, same padding/shadow) since it sits directly under
// it on the dashboard home, so the two read as a matched pair rather than
// two differently-styled UI patterns stacked on top of each other.
export default function PersonalRecordsBar({ records }: { records: PersonalRecord[] }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 bg-surface-container-low pl-3 pr-4 py-2.5 shadow-hard mb-6 hover:border-primary-container border border-transparent transition-colors"
    >
